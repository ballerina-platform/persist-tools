...
public isolated client class H2Client {
    *persist:AbstractPersistClient;
    ...
    public isolated function init(string url, string? user = (), string? password = (), jdbc:Options? connectionOptions = ()) returns persist:Error? {
        ...
        self.persistClients = {
            [ALL_TYPES]: check new (dbClient, self.metadata.get(ALL_TYPES), psql:H2_SPECIFICS),
            [STRING_ID_RECORD]: check new (dbClient, self.metadata.get(STRING_ID_RECORD), psql:H2_SPECIFICS),
            [INT_ID_RECORD]: check new (dbClient, self.metadata.get(INT_ID_RECORD), psql:H2_SPECIFICS),
            [FLOAT_ID_RECORD]: check new (dbClient, self.metadata.get(FLOAT_ID_RECORD), psql:H2_SPECIFICS),
            [DECIMAL_ID_RECORD]: check new (dbClient, self.metadata.get(DECIMAL_ID_RECORD), psql:H2_SPECIFICS),
            [BOOLEAN_ID_RECORD]: check new (dbClient, self.metadata.get(BOOLEAN_ID_RECORD), psql:H2_SPECIFICS),
            [COMPOSITE_ASSOCIATION_RECORD]: check new (dbClient, self.metadata.get(COMPOSITE_ASSOCIATION_RECORD), psql:H2_SPECIFICS),
            [ALL_TYPES_ID_RECORD]: check new (dbClient, self.metadata.get(ALL_TYPES_ID_RECORD), psql:H2_SPECIFICS),
            [USER]: check new (dbClient, self.metadata.get(USER), psql:H2_SPECIFICS),
            [POST]: check new (dbClient, self.metadata.get(POST), psql:H2_SPECIFICS),
            [FOLLOW]: check new (dbClient, self.metadata.get(FOLLOW), psql:H2_SPECIFICS),
            [COMMENT]: check new (dbClient, self.metadata.get(COMMENT), psql:H2_SPECIFICS),
            [EMPLOYEE]: check new (dbClient, self.metadata.get(EMPLOYEE), psql:H2_SPECIFICS),
            [WORKSPACE]: check new (dbClient, self.metadata.get(WORKSPACE), psql:H2_SPECIFICS),
            [BUILDING]: check new (dbClient, self.metadata.get(BUILDING), psql:H2_SPECIFICS),
            [DEPARTMENT]: check new (dbClient, self.metadata.get(DEPARTMENT), psql:H2_SPECIFICS),
            [ORDER_ITEM]: check new (dbClient, self.metadata.get(ORDER_ITEM), psql:H2_SPECIFICS)
        };
    }
