...
public client class EntitiesClient {
    *persist:AbstractPersistClient;
    ...
    public function init() returns persist:Error? {
        ...
        self.persistClients = {medicalneeds: check new (self.dbClient, self.metadata.get(MEDICAL_NEEDS))};
    }
    ...
    isolated resource function get medicalneeds/[int itemId]/[int needId]() returns MedicalNeed|persist:Error {
        MedicalNeed|error result = (check self.persistClients.get(MEDICAL_NEEDS).runReadByKeyQuery(MedicalNeed, {itemId: itemId, needId: needId})).cloneWithType(MedicalNeed);
        if result is error {
    ...
    isolated resource function put medicalneeds/[int itemId]/[int needId](MedicalNeedUpdate value) returns MedicalNeed|persist:Error {
        _ = check self.persistClients.get(MEDICAL_NEEDS).runUpdateQuery({"itemId": itemId, "needId": needId}, value);
    ...
    isolated resource function delete medicalneeds/[int itemId]/[int needId]() returns MedicalNeed|persist:Error {
        ...
        _ = check self.persistClients.get(MEDICAL_NEEDS).runDeleteQuery({"itemId": itemId, "needId": needId});
        return result;
    }
