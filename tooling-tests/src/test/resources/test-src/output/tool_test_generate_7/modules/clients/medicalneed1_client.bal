...
public client class MedicalNeed1Client {
    ...
    remote function read(map<anydata>? filter = ()) returns stream<MedicalNeed1, error?> {
        stream<anydata, error?>|error result = self.persistClient.runReadQuery(MedicalNeed1, filter);
        if result is error {
