...
public client class MedicalNeed1Client {
    ...
    remote function read(map<anydata>? filter = ()) returns stream<MedicalNeed1, error?>|error {
        stream<anydata, error?> result = check self.persistClient.runReadQuery(MedicalNeed1, filter);
