...
client class MedicalNeed2Client {
    ...
    remote function create(MedicalNeed2 value) returns int|persist:Error? {
        sql:ExecutionResult result = check self.persistClient.runInsertQuery(value);
        return <int>result.lastInsertId;
    }

    remote function readByKey(int key) returns MedicalNeed2|persist:Error {
        return (check self.persistClient.runReadByKeyQuery(MedicalNeed2, key)).cloneWithType(MedicalNeed2);
    }

    remote function read(map<anydata>? filter = ()) returns stream<MedicalNeed2, persist:Error?>|persist:Error {
        stream<anydata, error?> result = check self.persistClient.runReadQuery(filter);
        return new stream<MedicalNeed2, error?>(new MedicalNeed2Stream(result));
    }
...
public class MedicalNeed2Stream {
    ...
    public isolated function next() returns record {|MedicalNeed2 value;|}|persist:Error? {
        ...
        if streamValue is () {
            ...
            record {|MedicalNeed2 value;|} nextRecord = {value: check streamValue.value.cloneWithType(MedicalNeed2)};
            return nextRecord;
        }
    }
