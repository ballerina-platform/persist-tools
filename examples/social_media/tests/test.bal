...
//     redis:Client dbClient = check new (redis);
// }
...
function testGetUsers() returns error? {
    ...
    test:assertEquals(result.getJsonPayload(), [{"firstName": "Dinuka", "lastName": "Ashan", "email": "dinuka@gmail.com"}], "User details should be returned");
}
...
function testGetPosts() returns error? {
  ...
  test:assertEquals(result.statusCode, 200, "Status code should be 200");
  test:assertEquals(result.getJsonPayload(), [{id: 1,
    "title": "Post Exists",
    "content": "Content Exists",
    "userFirstName": "Dinuka",
    "userLastName": "Ashan",
...
function testCreateComment() returns error? {
  entities:CommentInsert comment = {
    ...
    message: "Comment 1",
    postId: 1,
    timeStamp: {utcOffset: {hours: 5, minutes: 30, seconds: 0}, timeAbbrev: "Asia/Colombo", year: 2024, month: 2, 
  ...
  test:assertEquals(result.getJsonPayload(), [
    {
      "id": 1,
      "message": "Comment 1",
      "timeStamp": {"utcOffset": {"hours": 5, "minutes": 30, "seconds": 0}, "timeAbbrev": "Asia/Colombo", "year": 2024, "month": 2, "day": 27, "hour": 10, "minute": 30, "second": 21},
      "post": {
        "id": 1,
        "content": "Content 1"
      }
...
@test:Config{
  dependsOn: [testCreatePostAlreadyExists, testGetPosts]
}
...
@test:Config{
  dependsOn: [testCreatePostAlreadyExists, testGetComments]
}
...
function testDeleteUser() returns error? {
  http:Response result = check socialMediaEndpoint->/users/[1].delete();
  test:assertEquals(result.statusCode, 204, "Status code should be 204");
  http:Response result2 = check socialMediaEndpoint->/users/[1];
...
function testDeletePost() returns error? {
  ...
  test:assertEquals(result.statusCode, 204, "Status code should be 204");
  http:Response result2 = check socialMediaEndpoint->/posts/[1];
  test:assertEquals(result2.statusCode, 200, "Status code should be 200");
...
function testDeleteComment() returns error? {
  ...
  test:assertEquals(result.statusCode, 204, "Status code should be 204");
  http:Response result2 = check socialMediaEndpoint->/comments/[1];
  test:assertEquals(result2.statusCode, 200, "Status code should be 200");
